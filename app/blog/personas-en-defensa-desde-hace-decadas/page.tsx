import type { Metadata } from "next";
import Link from "next/link";
import { contactWhatsApp } from "@/lib/content";
import { ArticleStructuredData } from "@/components/structured-data";
import { getBlogAlternates } from "@/lib/blog-helpers";
import { RelatedArticles } from "@/components/related-articles";

export const metadata: Metadata = {
  title: "Hay personas que viven en defensa desde hace décadas",
  description: "Cuerpos tensos. Respiraciones cortas. Mandíbulas apretadas. Vigilancia constante.",
  alternates: getBlogAlternates("es", "personas-en-defensa-desde-hace-decadas"),
};

export default function ArticlePage() {
  return (
    <div className="min-h-screen" style={{ background: "var(--bg-main)", color: "var(--text-main)" }}>
      <ArticleStructuredData
        title="Hay personas que viven en defensa desde hace décadas"
        description="Cuerpos tensos. Respiraciones cortas. Mandíbulas apretadas. Vigilancia constante."
        slug="personas-en-defensa-desde-hace-decadas"
        date="2026-10-23"
        tag="Manifiesto"
        lang="es"
      />
      <header className="sticky top-0 z-30" style={{ borderBottom: "1px solid var(--border-subtle)", background: "var(--bg-main)" }}>
        <div className="mx-auto flex max-w-[1100px] items-center justify-between px-8 py-5">
          <Link href="/" className="leading-none">
            <div className="text-[17px] font-semibold tracking-[-0.01em]" style={{ color: "var(--text-main)" }}>Ferran Moreno</div>
            <div className="mt-[5px] text-[10px] uppercase tracking-[0.14em]" style={{ color: "var(--text-secondary)", opacity: 0.7 }}>
              TRABAJO CORPORAL · REGULACIÓN · ENERGÍA
            </div>
          </Link>
          <nav className="hidden items-center gap-8 md:flex" style={{ color: "var(--text-secondary)" }}>
            <Link href="/" className="text-[11px] uppercase tracking-[0.08em] transition hover:opacity-50">Inicio</Link>
            <Link href="/wall-of-trust" className="text-[11px] uppercase tracking-[0.08em] transition hover:opacity-50">Wall of trust</Link>
            <Link href="/historia" className="text-[11px] uppercase tracking-[0.08em] transition hover:opacity-50">Mi historia</Link>
            <Link href="/blog" className="text-[11px] uppercase tracking-[0.08em] transition hover:opacity-50">Blog</Link>
            <a href={contactWhatsApp} target="_blank" rel="noopener noreferrer" className="text-[11px] uppercase tracking-[0.08em] transition hover:opacity-50">Contacto</a>
            <span style={{ color: "var(--border-subtle)" }}>|</span>
            <Link href="/en" className="text-[11px] uppercase tracking-[0.08em] transition hover:opacity-50">EN</Link>
          </nav>
        </div>
      </header>

      <main className="mx-auto max-w-[700px] px-6 py-24">
        <Link href="/blog" className="text-[11px] uppercase tracking-[0.15em] mb-12 inline-block transition hover:opacity-50" style={{ color: "var(--text-secondary)" }}>
          &larr; Blog
        </Link>

        <article>
          <span className="text-[10px] uppercase tracking-[0.15em] mb-4 block" style={{ color: "var(--text-secondary)" }}>
            Manifiesto
          </span>

          <h1 className="text-3xl font-semibold tracking-tight mb-10 leading-[1.15] lg:text-4xl">
            Hay personas que viven en defensa desde hace décadas
          </h1>

          <div className="space-y-6">
            <p className="text-[15px] leading-relaxed">
              Cuerpos tensos.<br />
              Respiraciones cortas.<br />
              Mandíbulas apretadas.<br />
              Vigilancia constante.
            </p>

            <p className="text-[15px] leading-relaxed">
              El sistema aprendió a sobrevivir.
            </p>

            <p className="text-[15px] leading-relaxed">
              Y olvidó cómo descansar.
            </p>

            <p className="text-[15px] leading-relaxed">
              Muchas de esas personas funcionan extraordinariamente bien. Trabajan. Producen. Cuidan de otros. Resuelven.
            </p>

            <p className="text-[15px] leading-relaxed">
              Pero todo desde defensa.
            </p>

            <p className="text-[15px] leading-relaxed">
              Y la defensa permanente tiene un coste que se paga en silencio:
            </p>

            <p className="text-[15px] leading-relaxed">
              Menos energía.<br />
              Menos flexibilidad.<br />
              Menos presencia.<br />
              Menos vida.
            </p>

            <p className="text-[15px] leading-relaxed">
              El cuerpo puede vivir así durante décadas.
            </p>

            <p className="text-[15px] leading-relaxed">
              Pero eso no significa que esté bien.
            </p>

            <p className="text-[15px] leading-relaxed">
              Significa que es brillante sobreviviendo.
            </p>

            <p className="text-[15px] leading-relaxed">
              La pregunta es cuánto tiempo más quieres vivir sobreviviendo.
            </p>
          </div>

          <div className="mt-20 pt-12" style={{ borderTop: "1px solid var(--border-subtle)" }}>
            <p className="text-[15px] leading-relaxed mb-6" style={{ color: "var(--text-secondary)" }}>
              Si algo de lo que has leído te resuena, escríbeme.
            </p>
            <a
              href={contactWhatsApp}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block text-[11px] uppercase tracking-[0.15em] font-medium transition hover:opacity-50"
              style={{ color: "var(--text-main)" }}
            >
              Escríbeme &rarr;
            </a>
          </div>
          <RelatedArticles slug="personas-en-defensa-desde-hace-decadas" lang="es" />
        </article>
      </main>
    </div>
  );
}
